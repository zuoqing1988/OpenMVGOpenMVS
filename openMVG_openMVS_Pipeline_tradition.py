...
cmdline = ZQ_utils.aug_path(os.path.join(OPENMVG_BIN, 'openMVG_main_IncrementalSfM.exe'))+' -i '+ZQ_utils.aug_path(os.path.join(matches_dir,'sfm_data.json'))+' -m '+ZQ_utils.aug_path(matches_dir)+ ' -o '+ZQ_utils.aug_path(reconstruction_dir)+' -T -c 2 -f "ADJUST_FOCAL_LENGTH|ADJUST_DISTORTION"'
print(cmdline)
...
time3 = time.time()
print('openmMVS cost time: %f'%(time3-time2))
